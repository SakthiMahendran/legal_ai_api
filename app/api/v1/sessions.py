...
from legal_ai_api.app.models.pydantic_schemas import SessionModel, CreateSessionRequest
from fastapi.responses import JSONResponse
from legal_ai_api.app.services.session_service import SessionService
...
async def update_session(id: int, request: CreateSessionRequest, db: AsyncSession = Depends(get_db)):
    session = await session_service.update_session(db, id, request.dict())
    return session

@router.patch('/{id}/', response_model=SessionModel)
async def patch_session(id: int, request: CreateSessionRequest, db: AsyncSession = Depends(get_db)):
    session = await session_service.update_session(db, id, request.dict(exclude_unset=True))
    return session
