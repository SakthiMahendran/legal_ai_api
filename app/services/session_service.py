...
from legal_ai_api.app.models.sql_models import Session as SessionModel
from legal_ai_api.app.models.sql_models import User as UserModel
from sqlalchemy import update
from datetime import datetime

class SessionService:
    async def create_session(self, db: AsyncSession, title: str, status: str = "active", user_id: int = None):
    ...
    async def update_session(self, db: AsyncSession, session_id: int, updates: dict):
        await db.execute(update(SessionModel).where(SessionModel.id == session_id).values(**updates, updated_at=datetime.utcnow()))
        await db.commit()
