...
from legal_ai_api.app.models.sql_models import Document as DocumentModel
from sqlalchemy import update
from datetime import datetime

class DocumentService:
    async def create_document(self, db: AsyncSession, session_id: int, title: str, filename: str, file_path: str, status: str = "active"):
        document = DocumentModel(session_id=session_id, title=title, filename=filename, file_path=file_path, status=status)
        db.add(document)
        await db.commit()
    ...
    async def update_document(self, db: AsyncSession, document_id: int, updates: dict):
        await db.execute(update(DocumentModel).where(DocumentModel.id == document_id).values(**updates, updated_at=datetime.utcnow()))
        await db.commit()
